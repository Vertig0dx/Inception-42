...
if [ $RETRY_COUNT -eq 0]; then
    echo "wp core download failure after 1 attempt, aborting."
    exit 1
fi
...
#edits the default wp-config.php file with the variables inside a .env file
...
wp core install --url=$DOMAIN_NAME/ --title=$WP_TITLE --admin-user=$WP_ADMIN_USER --admin-password=$WP_ADMIN_PWD --admin-email=$WP_ADMIN_EMAIL --skip-email --allow-root

#create a regular user in addition to the admin created during final wordpress instalation

wp user create $WP_USER $WP_EMAIL --role=author --user-pass=$WP_PWD --allow-root
